...
def banking_anomalies(token, url):
  ...
  response = requests.get(url + "banking/anomalies/625f965bc4b57cdb24ef5e4b?query=&sortKey=status&sortOrder=1&page=&size=", headers=headers)
...
def banking_anomalies_export(token, url):
  ...
  response = requests.get(url + "banking/anomalies/625f965bc4b57cdb24ef5e4b/export?query=&sortKey=status&sortOrder=1", headers=headers)
  if response.status_code == 200:
    data = response.json()
    print(data)
  else:
...
def banking_payments_export(token, url):
  ...
  response = requests.get(url + "banking/payments/625f9719c4b57cdb24ef5e4d/export?query=&sortKey=&sortOrder=", headers=headers)
  if response.status_code == 200:
    data = response.json()
    print(data)
  else:
...
def banking_get_transaction_suppliername(token, url):
  ...
  response = requests.get(url + "banking/get-transaction?supplierName=f0be81e0-72a5-4234-92ad-85f5a9292&clientId=62fb53eb8af7efee9e56e08d", headers=headers)
  if response.status_code == 200:
    data = response.json()
    print(data)
  else:
