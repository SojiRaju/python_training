...
def banking_payments(token, url):
    ...
    response = requests.get(
        url + "banking/payments/64b8fb5bd8977cadb2f8b94b?query=&page=&size=18&sortKey=currencyCode&sortOrder=1",
        headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(data)
...
def banking_anomalies(token, url):
    ...
    response = requests.get(
        url + "banking/anomalies/64b8fb5bd8977cadb2f8b94b?query=&sortKey=status&sortOrder=1&page=&size=",
        headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(data)
...
def banking_get_transaction_suppliername(token, url):
    ...
    response = requests.get(
        url + "banking/get-transaction?supplierName=Edstem, Nithin&clientId=634fb054474a06e0a8c54b97", headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(data)
    else:
