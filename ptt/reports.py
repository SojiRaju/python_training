...
def trust_balance_report(token, url):
    ...
    response = requests.get(url + "reports/trust-balance?client=62fb53eb8af7efee9e56e08d&page=&size=&currency=GBP",
                            headers=headers)
...
def trust_balance_report_export(token, url):
    ...
    response = requests.get(url + "reports/trust-balance/export?client=62fb53eb8af7efee9e56e08d&page=1&size=10",
                            headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(data)
...
def client_files_report(token, url):
    ...
    response = requests.get(
        url + "reports/client-files?client=62fb53eb8af7efee9e56e08d&fromDate=2022-02-24&toDate=2022-02-24&page=1&size=3",
        headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(data)
